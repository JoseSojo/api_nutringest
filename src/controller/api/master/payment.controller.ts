...
export default class PaymentMethodController {
    ...
    @Get(`:id/unique/public`)
    @UseGuards(AuthGuard)
    private async uniquePublic(@Req() req: any, @Param() param: { id: string }) {
        const user = req.user as any;
        const permit = user.rolReference.roles as string[];
        const action = this.getPermit().list;
        ...
        // validar eliminación
        const filter: Prisma.PaymentMethodWhereInput = { id:param.id };
    ...
    private objectName() { return `payment` }
